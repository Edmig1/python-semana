...
def Cadastrar():
    ...
    if login and senha and nome and genero!='Selecione' and data:
        add_db(login,nome,cpf,genero,data,senha)
        Lb_Erro.configure(text='Cadastro Efetuado')


def Autenticar():
    login = Cx_Login.get()
    senha = Cx_Senha.get()
    resultado = autenticar(login,senha)
    match resultado:
        case 'sucesso':
            print('sucesso')
        case 'falhou':
            print('errado')
        case 'vazia':
            Lb_Erro.configure(text='Informações faltando')
